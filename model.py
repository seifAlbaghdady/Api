...
class BatchTreeEncoder(nn.Module):
    ...
    def traverse_mul(self, node, batch_index):
        ...
        batch_current = batch_current.index_copy(
            0,
            Variable(self.th.LongTensor(index).to(self.device)),
            self.embedding(Variable(self.th.LongTensor(current_node).to(self.device))),
        )  # do not need to transform if use GRU

        childs_hidden_sum = self.create_tensor(
        ...
        for c in range(len(children)):
            ...
            if tree is not None:
                cur_child_hidden = zeros.index_copy(
                    0,
                    Variable(self.th.LongTensor(children_index[c]).to(self.device)),
                    tree,
                )
        ...
        if self.use_att and (len(hidden_per_child) != 0):
            ...
            childs_weighted = matrix_mul(
                child_hiddens, self.sent_weight, self.sent_bias
            )
            childs_weighted = matrix_mul(childs_weighted, self.context_weight).permute(
                1, 0
            )
            childs_weighted = F.softmax(childs_weighted)
            childs_hidden_sum = element_wise_mul(
...
class BatchProgramClassifier(nn.Module):
    def __init__(
        ...
        self.encoder_layer = nn.TransformerEncoderLayer(
            d_model=self.encode_dim, nhead=4
        )
    ...
    def init_hidden(self):
        if self.gpu is True:
            if isinstance(self.bilstm, nn.LSTM):
                ...
                return h0, c0
            return Variable(
                torch.zeros(self.num_layers * 2, self.batch_size, self.hidden_dim).to(
                    self.device
                )
            )
        else:
